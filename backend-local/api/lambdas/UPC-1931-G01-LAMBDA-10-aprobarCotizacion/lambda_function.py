...
def lambda_handler(event, context):
    ...
    try:
        ...
        if row.estado != 'Pendiente':
            return {'statusCode': 400, 'body': json.dumps({"error": "Solo se pueden aprobar cotizaciones en estado Pendiente"})}

        prev_status_id = row.status_id

        cursor.execute("SELECT id FROM quotation_statuses WHERE name = 'Aprobada'")
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, 'Cotización aprobada')
        """, [id, prev_status_id, new_status_id, user_id])
        ...
        return {
            ...
            'body': json.dumps({"message": "Cotización aprobada correctamente"})
        }
