(function () {
  ...
  function initBuilder(root) {
    ...
    const rejectModal =
      rejectModalEl && window.bootstrap
        ? bootstrap.Modal.getOrCreateInstance(rejectModalEl)
        : null;
    ...
    function renderSidebar() {
      ...
      if (subtotalNode) subtotalNode.textContent = formatMoney(taxableBase());
      if (taxNode) taxNode.textContent = formatMoney(taxAmount());
    ...
    function renderPreview() {
      ...
      tableBody.innerHTML = getItemsArray()
        .map(
          item => `
        <tr>
          <td>
